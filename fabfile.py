...
from fabric.api import cd, env, execute, hide, local, puts, roles, run, sudo, task

env.project_name = "{{ project_name }}"
env.repository = "git@github.com:lincolnloop/{{ project_name }}.git"
env.local_branch = "main"
...
def bootstrap(action=""):
    ...
    if exists and not action == "force":
        puts(
            "Assuming {host} has already been bootstrapped since "
    ...
    requirements()
    puts(
        "Bootstrapped {host} - database creation needs to be done manually.".format(
            **env
        )
    )
...
def push():
    ...
    local(
        "git push {remote} {local_branch}:{dest_branch}".format(
...
def requirements():
    ...
    """
    run(
        "{virtualenv_dir}/bin/pip install -r {project_dir}/requirements.txt".format(
            **env
        )
    )
...
def dj(command):
    ...
    """
    run(
        "{virtualenv_dir}/bin/manage.py {dj_command} "
